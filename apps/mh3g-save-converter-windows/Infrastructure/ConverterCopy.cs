...
public sealed class ConverterCopy : ObservableObject
{
    ...
    public string RepairCecPathsRequired => Text(nameof(RepairCecPathsRequired));
    ...
    public string RepairSystemPathsRequired => Text(nameof(RepairSystemPathsRequired));
    public string ExtrasPathsRequired => Text(nameof(ExtrasPathsRequired));
    public string RepairExtrasPathsRequired => Text(nameof(RepairExtrasPathsRequired));
    ...
    public string SystemCurrent => Text(nameof(SystemCurrent));
    public string SystemTarget => Text(nameof(SystemTarget));
    public string SystemRepairOutput => Text(nameof(SystemRepairOutput));
    ...
    public string ExtDataCurrent => Text(nameof(ExtDataCurrent));
    public string ExtDataRepairOutput => Text(nameof(ExtDataRepairOutput));
    public string ExtDataRepairPathHint => Text(nameof(ExtDataRepairPathHint));
    public string ExtDataRepairOutputWarning => Text(nameof(ExtDataRepairOutputWarning));
    public string RepairGroupDryRun => Text(nameof(RepairGroupDryRun));
    public string RepairGroupWrite => Text(nameof(RepairGroupWrite));
    public string GuildCardsManifest => Text(nameof(GuildCardsManifest));
    public string QuestsManifest => Text(nameof(QuestsManifest));
    ...
    public string CecCurrentCache => Text(nameof(CecCurrentCache));
    public string CecRepairOutput => Text(nameof(CecRepairOutput));
    ...
    private static readonly IReadOnlyDictionary<string, string> English = new Dictionary<string, string>
    {
        ...
        [nameof(ConversionModeRepairDescription)] = "Repair is domain-scoped. Core user#, shared system, guild cards, quests, and experimental CEC each use their own Dry Run, write authorization, manifest, and rollback. Every enabled domain explicitly separates original 3DS source, current read-only Wii U / Cemu reference, and output.",
        ...
        [nameof(OptionalSectionDescription)] = "Shared system carries housekeeper gallery/movie unlocks; ExtData carries cards and quests. In repair mode, cards and quests are separate component-group transactions. CEC stays isolated and disabled until you enable it.",
        ...
        [nameof(CecCurrentCache)] = "Current Wii U / Cemu cec reference",
        [nameof(CecRepairOutput)] = "Repaired cec output",
        ...
        ["CecTargetCacheHint"] = "Choose the exact Cemu cec file, usually inside the selected Wii U save directory. Repair mode adds a separate read-only current cec reference and writes only the explicit output.",
        ...
        [nameof(ExtDataNotice)] = "ExtData is optional. New conversion uses complete staging plus selected-group install. Repair mode skips staging and repairs guild cards or quests independently from explicit source/current/output directories.",
        ...
        [nameof(RepairCecPathsRequired)] = "Choose the original 3DS CEC mailbox, current read-only Cemu cec reference, and repaired cec output.",
        ...
        [nameof(RepairSystemPathsRequired)] = "Choose the exact original 3DS system, current read-only Wii U / Cemu system, and repaired system output.",
        [nameof(ExtrasPathsRequired)] = "Choose at least one ExtData group plus explicit source and staging directories.",
        [nameof(RepairExtrasPathsRequired)] = "Choose at least one ExtData group plus explicit original 3DS source, current Wii U / Cemu reference, and initialized output directories.",
        ...
        [nameof(ExtDataCurrent)] = "Current Wii U / Cemu ExtData directory",
        [nameof(ExtDataRepairOutput)] = "Repaired ExtData output directory",
        [nameof(ExtDataRepairPathHint)] = "Repair uses three independent directories. Source is the original 3DS ExtData user directory; current is the latest Wii U / Cemu save directory you continued playing; output is the only directory written.",
        [nameof(ExtDataRepairOutputWarning)] = "The output must already be an initialized MH3G Wii U save directory containing the complete selected group. Guild cards require card1/card2/card3/cardbox; quests require quest1/quest2/quest3/quest4.",
        [nameof(RepairGroupDryRun)] = "Run independent Dry Run",
        [nameof(RepairGroupWrite)] = "Write this group",
        [nameof(GuildCardsManifest)] = "Guild-card repair manifest",
        [nameof(QuestsManifest)] = "Quest repair manifest",
    ...
    private static readonly IReadOnlyDictionary<string, string> Chinese = new Dictionary<string, string>
    {
        ...
        [nameof(ConversionModeRepairDescription)] = "修复按数据域独立执行：核心 user#、共享 system、公会名片、任务和实验性 CEC 都有自己的 Dry Run、写入授权、manifest 与回滚。每个启用的数据域都明确分开原始 3DS 来源、当前只读 Wii U / Cemu 引用和输出。",
        ...
        [nameof(OptionalSectionDescription)] = "共享 system 包含管家画廊/动画解锁记录，ExtData 包含名片与任务；修复模式下名片和任务也是两个独立组件组事务。CEC 完全隔离，默认关闭。",
        ...
        [nameof(CecCurrentCache)] = "当前 Wii U / Cemu cec 只读引用",
        [nameof(CecRepairOutput)] = "修复后的 cec 输出",
        ["CecSourceDirectoryHint"] = "请选择准确的 CEC/00048100 邮箱目录。它与 ExtData user 目录独立，应用不会递归搜索。",
        ["CecTargetCacheHint"] = "请选择准确的 Cemu cec 文件，通常位于所选 Wii U 存档目录中。修复模式会额外要求一个只读当前 cec 引用，并且只写入明确选择的输出。",
        ...
        [nameof(ExtDataNotice)] = "ExtData 是可选项。全新转换使用完整暂存目录再安装所选组件组；修复模式不复用暂存流程，而是从明确的来源/当前引用/输出目录分别独立修复公会名片或任务。",
        ...
        [nameof(RepairCecPathsRequired)] = "请选择原始 3DS CEC 邮箱、当前只读 Cemu cec 引用，以及修复后的 cec 输出。",
        ...
        [nameof(RepairSystemPathsRequired)] = "请选择准确的原始 3DS system、当前只读 Wii U / Cemu system，以及修复后的 system 输出。",
        [nameof(ExtrasPathsRequired)] = "请至少选择一个 ExtData 组，以及准确的源目录和临时目录。",
        [nameof(RepairExtrasPathsRequired)] = "请至少选择一个 ExtData 组件组，并分别指定原始 3DS 来源、当前 Wii U / Cemu 只读引用，以及已初始化的输出目录。",
        ...
        [nameof(SystemCurrent)] = "当前 Wii U / Cemu system 只读引用",
        [nameof(SystemTarget)] = "已存在且已初始化的 Cemu system 文件",
        [nameof(SystemRepairOutput)] = "修复后的 system 输出",
        [nameof(SystemMergeWarning)] = "system 由三个角色槽位共享，可能同时保存其他槽位数据或设置。修复模式下“当前引用”只读、“输出”独立；转换器只把管家画廊/动画标记合入当前 Wii U 数据，其他当前字节全部保持不变。",
        ...
        [nameof(ExtDataCurrent)] = "当前 Wii U / Cemu ExtData 目录",
        [nameof(ExtDataRepairOutput)] = "修复后的 ExtData 输出目录",
        [nameof(ExtDataRepairPathHint)] = "修复使用三个相互独立的目录：来源是原始 3DS ExtData user 目录；当前引用是继续游玩后的最新 Wii U / Cemu 存档目录；只有输出目录会被写入。",
        [nameof(ExtDataRepairOutputWarning)] = "输出必须是已经初始化的 MH3G Wii U 存档目录，并完整包含所选组件组。公会名片需要 card1/card2/card3/cardbox；任务需要 quest1/quest2/quest3/quest4。",
        [nameof(RepairGroupDryRun)] = "独立运行 Dry Run",
        [nameof(RepairGroupWrite)] = "写入此组件组",
        [nameof(GuildCardsManifest)] = "公会名片修复 manifest",
        [nameof(QuestsManifest)] = "任务修复 manifest",
