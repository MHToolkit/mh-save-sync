...
public enum ConverterCopy {
    private static let zhHans: [String: String] = [
        ...
        "Components.RepairExtData": "修复模式会明确分开原始 3DS ExtData、当前只读 Wii U/Cemu 目录和输出目录；公会名片与任务分别进行 Dry Run、写入、manifest 和回滚。",
        "Input.Slot": "存档槽位",
        ...
        "Status.Detail.OptionalDataBlocked": "已启用的 system 或 ExtData 缺少明确路径；只禁用该数据域，核心角色及其他已授权数据域保持独立。",
        ...
        "Guide.OptionalDataNeedsConfiguration": "已启用的可选数据尚未配齐；这只会禁用该数据域的操作，不会阻止核心角色 Dry Run 或其他独立数据域。",
        ...
        "Components.SystemWarning": "system 由三个角色槽位共享。修复时当前 Wii U/Cemu system 只读、输出独立；转换器只合并管家画廊/动画标记，其他当前设置与跨槽位数据保持不变。",
        "Components.SystemFooter": "全新转换需要 3DS 源和已初始化的 Cemu 目标；修复需要原始 3DS、当前只读 Cemu 引用和输出，且拥有独立 Dry Run 与 manifest。",
        "Components.SystemSourceMessage": "请选择一个 3DS system 文件。",
        "Components.SystemTargetMessage": "请选择一个 Cemu system 文件。",
        "Components.ExtrasFooter": "3DS 来源可选择 00000481 或其直接 user 子目录；当前引用和输出选择准确的 Wii U 存档目录。应用只处理本次单独授权的完整名片组或任务组。",
        ...
        "Components.TargetMessage": "请选择准确的 Cemu MH3G 存档目录。所选组件组必须完整：名片为 card1/card2/card3/cardbox，任务为 quest1 至 quest4。",
    ...
    private static let english: [String: String] = [
        ...
        "Components.RepairExtData": "Repair explicitly separates original 3DS ExtData, the current read-only Wii U/Cemu directory, and output. Guild cards and quests each have their own Dry Run, write, manifest, and rollback.",
        ...
        "Status.Detail.OptionalDataBlocked": "A selected system or ExtData domain lacks an explicit path. Only that domain is disabled; core and other independently authorized domains remain available.",
        ...
        "Guide.OptionalDataNeedsConfiguration": "A selected optional domain is incomplete. This disables only that domain; it does not block the core Dry Run or another independent domain.",
        ...
        "Components.SystemSource": "3DS system",
        "Components.SystemTarget": "Cemu system",
        "Components.SystemWarning": "System is shared by all three character slots. During repair, the current Wii U/Cemu system is read-only and output is independent; only housekeeper gallery/movie flags are merged while every other current setting and shared-slot byte is retained.",
        "Components.SystemFooter": "New conversion needs a 3DS source and initialized Cemu target. Repair needs original 3DS, current read-only Cemu reference, and output, with its own Dry Run and manifest.",
        ...
        "Components.ExtrasFooter": "For the 3DS source choose 00000481 or its direct user child; current and output are exact Wii U save directories. The app handles only the independently authorized complete card or quest group.",
        ...
        "Components.TargetMessage": "Choose the exact Cemu MH3G save directory. Each selected group must be complete: card1/card2/card3/cardbox for cards, or quest1 through quest4 for quests.",
