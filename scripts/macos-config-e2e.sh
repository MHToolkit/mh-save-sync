...
grep -q "同步存档…" "$tmp/menu-preview.txt"
grep -q "服务器、存档目录和恢复密钥放在「设置」" "$tmp/menu-preview.txt"
grep -q "上传本地存档" "$tmp/menu-preview.txt"
grep -q "用云端恢复本地…" "$tmp/menu-preview.txt"
grep -q "本地设为云端最新（显式选择，旧版保留）" "$tmp/menu-preview.txt"
grep -q "历史版本.*尚未接入" "$tmp/menu-preview.txt"
