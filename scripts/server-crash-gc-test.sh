...
# Let Docker reserve the loopback port atomically.  Choosing an unused port in
# Python and binding it later leaves a TOCTOU window on shared CI hosts.
"$runtime" run -d --rm \
  --name "$container" \
...
port="$("$runtime" port "$container" 5432/tcp | awk -F: 'END { print $NF }')"
if [[ ! "$port" =~ ^[0-9]+$ ]]; then
  echo "could not resolve the crash/GC test PostgreSQL host port" >&2
  "$runtime" ps -a --filter "name=$container" >&2 || true
...
if [[ "$ready" != true ]]; then
  echo "crash/GC test PostgreSQL container did not become ready within 60 seconds" >&2
  "$runtime" ps -a --filter "name=$container" >&2 || true
  "$runtime" logs "$container" >&2 || true
  exit 1
fi
