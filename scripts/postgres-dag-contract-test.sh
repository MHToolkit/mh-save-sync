...
test_name="persistent_backend_enforces_multi_device_dag_contract"
if [[ -n "${MH_SAVE_SYNC_DAG_TEST_DATABASE_URL:-}" ]]; then
  DATABASE_URL="$MH_SAVE_SYNC_DAG_TEST_DATABASE_URL" \
...
if [[ -n "${CONTAINER_RUNTIME:-}" ]]; then
  ...
  echo "PostgreSQL DAG contract test requires MH_SAVE_SYNC_DAG_TEST_DATABASE_URL, podman, or docker" >&2
  exit 1
fi
...
container="mh-save-sync-dag-contract-test-$$"
cleanup() {
