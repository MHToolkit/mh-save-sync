...
### 2026-07-12 real Mac automatic-upload conflict RCA

- PostgreSQL DAG evidence showed the phone device advanced HEAD to `…f0f9f0` from parent `…0f69f8`; the subsequent Mac automatic upload produced branch `…27d6d029` with no parent. A later explicit Mac choice advanced HEAD to `…478f16` with parent `…f0f9f0`.
- Authenticated E2EE downloads were restored only into isolated temporary directories. The phone HEAD and Mac branch contained different save bytes, while the Mac branch and the later selected Mac HEAD were byte-identical. The original conflict was therefore a real divergence, not an mtime false positive.
