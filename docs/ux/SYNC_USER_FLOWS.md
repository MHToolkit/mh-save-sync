...
- Android UI 已改为中文同步工作台，能显示服务器目标、SAF 授权、MH3G 开关、真实 `/ready` + MH3G HEAD 启动前探测、冲突选择、游戏运行保护、恢复云端到本地（需停止 Nemessix）与后台策略；同步动作区提供明确的「本地替换云端（保留云端旧版本）」入口，运行中只记录选择并等待退出后的稳定校验，避免上传正在写入的中间态。
