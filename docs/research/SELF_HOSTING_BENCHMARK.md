...
| Orphan GC pagination and recovery | Create 1,005 S3-only objects plus one PostgreSQL-tracked orphan and one referenced object; run real CLI dry-run/delete, enumerate MinIO versions, then stop MinIO. | listing crosses one S3 page; exact logical/physical counts; PostgreSQL and MinIO version cross-check; no storage key in stderr. | **Passed locally.** Dry-run reported 1,006; logical delete and version-aware purge each reported 1,006; mark and physical-purge queues returned to zero; `mc find --versions` retained exactly the referenced live version with no orphan noncurrent data/delete markers; unavailable MinIO produced only the redacted `object-store unavailable` code. `scripts/orphan-gc-compose-e2e.sh`. |
...
- Versioned physical purge: migration 006 records a second durable queue.
  Compose streams leased keys to MinIO's version-aware removal over stdin and
  acknowledges only after success. Generic S3 deployments must configure an
